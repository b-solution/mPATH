    ...
    <div class="col-md-10">
      <div class="right-panel">
        ...
        <div class="my-1 pb-2 buttonWrapper container-fluid">
          <div class="row px-0">
            <div class="col-6">
              <el-button @click.prevent="addGroup" v-if="_isallowed('write')" class="bg-primary text-light mb-2">
                <i class="far fa-plus-circle mr-1"></i> Create Group
              </el-button>
              <el-button @click.prevent="openPortfolioGroup" v-if="_isallowed('write')"
                class="bg-success text-light mb-2">
                <i class="far fa-plus-circle mr-1"></i> Add Portfolio Group(s)
              </el-button>
            </div>
            <div class="col-6">
              <el-input type="search" placeholder="Search Groups" aria-label="Search" aria-describedby="search-addon"
        ...
        <el-dialog :visible.sync="dialogVisible" append-to-body center class="contractForm p-0 createNewGroup">
          ...
          <form accept-charset="UTF-8">
            <div class="form-group mx-3">
              <el-input v-model="newGroupName" placeholder="Enter new Group Name" rows="1" name="Group Name" />
            </div>
            <div class="right mr-2">
              <button @click.prevent="createNewGroup" v-show="newGroupName"
                class="btn btn-md bg-primary text-light  modalBtns" v-tooltip="`Save New Group`"
                :class="[hideSaveBtn ? 'd-none' : '']">
                <i class="fal fa-save"></i>
              </button>
              <button @click.prevent="cancelCreateGroup" class="btn btn-md bg-secondary text-light modalBtns"
                v-tooltip="`Cancel`" :class="[hideSaveBtn ? 'd-none' : '']">
                <i class="fas fa-ban"></i>
              </button>
              <button @click.prevent="addAnotherGroup" v-tooltip="`Add Another Group`"
                :class="[!hideSaveBtn ? 'd-none' : '']" class="btn btn-md bg-primary text-light modalBtns">
                <i class="far fa-plus-circle"></i>
              </button>
              <button @click.prevent="closeAddGroupBtn" v-tooltip="`Cancel`"
                class="btn btn-md bg-secondary text-light modalBtns mr-2" :class="[!hideSaveBtn ? 'd-none' : '']">
                <i class="fas fa-ban"></i>
              </button>
            </div>
          </form>
        ...
        <el-dialog :visible.sync="dialog2Visible" append-to-body center class="portfolioNames p-0"
          ...
          <div>
              ...
              <div class="sticky">
                <div class="row">
                  <div slot="title" class="col-8 pr-0 text-left">
                    <h5 class="text-dark addGroupsHeader"> <i class="fal fa-network-wired mr-2 mh-blue-text"></i>Select
                      Portfolio Group(s) to Add </h5>
                  </div>
                  <div class="col-7 pt-0 text-left">
                    <el-input type="search" placeholder="Search Groups" aria-label="Search" class="w-100"
                      aria-describedby="search-addon" v-model="searchGroups" data-cy="">
                      <el-button slot="prepend" icon="el-icon-search"></el-button>
                    </el-input>
                  </div>
                  <div class="col pt-0 text-right">
                    <el-button class="confirm-save-group-names btn text-light bg-primary modalBtns"
                      v-tooltip="`Save Group(s)`" @click.prevent="importGroupName"
                      :disabled="portfolioGroups.length <= 0">
                      <i class="fal fa-save"></i>
                    </el-button>
                    <el-button @click.prevent="closeImportGroupBtn" v-tooltip="`Cancel`"
            ...
            <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange"><i>Check all
                Groups</i></el-checkbox>
            <div style="margin: 15px 0;"></div>
            <el-checkbox-group v-model="checkedPortfolioGroups">
              <div class="row">
                <div class="col-4">
                  <el-checkbox v-for="group in portfolioGroups.filter(g => g.is_portfolio)" :label="group.id"
                    class="d-flex" :key="group.id">{{ group.name }}</el-checkbox>
                </div>
              </div>
            </el-checkbox-group>
          </div>
        ...
        <div class="container-fluid mt-2 mx-0">
          <div v-loading="!contentLoaded" v-if="_isallowed('read')"
            element-loading-text="Fetching your data. Please wait..." element-loading-spinner="el-icon-loading"
            element-loading-background="rgba(0, 0, 0, 0.8)" class="mt-2">
            <el-table v-if="tableData" :data="tableData
              ...
              .reverse()
            " style="width: 100%" height="475" highlight-current-row
              ...
              <el-table-column prop="name" sortable label="Groups">
                <template slot-scope="props">
                  <div class="row">
                    <div class="col-9">
                      <el-input size="small" v-if="rowId == props.row.id" style="text-align:center"
                        v-model="props.row.name" controls-position="right">
                      </el-input>
                      <span v-else>
                        <span v-if="props.row.isPortfolio" v-tooltip="`Portfolio Group`"><i
                ...
                <template slot-scope="props">
                  <div class="row">
                    <div class="col">
                      ...
                      <span class="mr-3" v-if="groupProjects &&
            groupProjects
            ...
            groupProjects
              ...
              .filter((t) => t == props.row.id).length
                      ...
                      <span class="mr-3" v-if="groupContracts && groupContracts.length > 0 &&
            groupContracts
            ...
            groupContracts
              ...
              .filter((t) => t == props.row.id).length
                      ...
                      <i class="far fa-car mr-1 text-info" v-tooltip="`Vehicles`"></i>
                    ...
                    <div class="row">
                      <div class="col">
                        <h5 class="mh-orange-text">
                          Projects
                          <span :load="log(groupProjects)" v-if="groupProjects &&
            groupProjects
              .map((t) => t.facilityGroupId)
            ...
            groupProjects
              ...
              .filter((t) => t == props.row.id).length
                          ...
                          <span v-else class="badge badge-secondary badge-pill pill">{{ 0 }}
                          </span>
                        </h5>
                        <span v-if="groupProjects">
                          <ul class="pl-3 mb-0" v-for="(item, i) in groupProjects.filter(
            ...
            groupContracts
              ...
              .filter((t) => t == props.row.id).length
                        ...
                        <span v-if="groupContracts && groupContracts.length > 0">
                          <ul class="pl-3 mb-0">
                            <li v-for="(item, i) in groupContracts.filter(
            ...
            groupVehicles
              ...
              .filter((t) => t == props.row.id).length
                        ...
                        <span v-if="groupVehicles && groupVehicles.length > 0">
                          <ul class="pl-3 mb-0">
                            <li v-for="(item, i) in groupVehicles.filter(
            (t) => t.facility_group_id == props.row.id
              ...
              <el-table-column label="Actions" align="right" v-if="_isallowed('delete') || _isallowed('write')">
                <template slot-scope="scope">
                  <el-button size="small" type="default" v-tooltip="`Save`"
                    @click.prevent="saveEdits(scope.$index, scope.row)" v-if="scope.$index == rowIndex
            ...
            " @click.prevent="cancelEdits(scope.$index, scope.row)" class="bg-secondary  btn-sm text-light">
                  ...
                  <el-button size="small" type="default" v-tooltip="`Edit Program Group`"
                    @click.prevent="editMode(scope.$index, scope.row)" v-if="!scope.row.isPortfolio &&
            scope.$index !== rowIndex && _isallowed('write')
            " class="bg-light btn-sm ">
                    <i class="fal fa-edit text-primary"></i>
                  </el-button>
                  <el-button size="small" type="default" class="bg-light btn-sm" v-tooltip="'Remove Portfolio Group'"
                    @click.prevent="removeGroup(scope.$index, scope.row)" v-if="scope.$index !== rowIndex &&
            scope.row.isPortfolio && _isallowed('delete')
            ">
                    <i class="fa-light fa-circle-minus text-danger"></i>
                  </el-button>
                  <el-button size="small" type="default" v-tooltip="'Delete Program Group'"
                    @click.prevent="removeGroup(scope.$index, scope.row)" v-if="!scope.row.isDefault &&

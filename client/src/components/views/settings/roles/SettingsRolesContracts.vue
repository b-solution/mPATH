<template>
  <div>
    <div :class="{ 'd-none': !_isallowed('write') }">
      <el-button @click.prevent="addContractRole" :disabled="isEditting" class="bg-primary text-light mb-3">
    ...
    <el-table v-if="tableData && tableData.length > 0" :data="tableData" height="450" class="crudRow">
      <el-table-column fixed prop="role" label="Contract/Vehicle Role" width="250">
        ...
        <template slot-scope="scope">
          ...
          <span v-else>
            <span v-show="scope.row.name == 'update-contract' ||
      scope.row.name == 'read-contract' ||
      scope.row.name == 'contribute-contract'" style="color: #dc3545; font-size: 15px">*
            </span>
            {{ scope.row.name }}
          </span>
        </template>
      ...
      <el-table-column fixed="right" label="Actions" class="text-center " width="120">
        <template slot-scope="scope">
          <el-button size="small" type="default" v-tooltip="`Save role`" v-if="(isEditting && currentRow == scope.$index) ||
      ...
      scope.row.name !== 'read-contract' &&
      scope.row.name !== 'contribute-contract'" class="bg-light btn-sm" v-tooltip="`Edit Role`">

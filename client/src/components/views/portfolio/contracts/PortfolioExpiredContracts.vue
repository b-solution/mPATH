  ...
  <div style="height:80vh" class="portfolio-contracts-module">
    <div style="height: 100%; overflow-y:auto" v-if="contractProjects && contractProjects.length > 0">
      <el-table
        ...
        border height="800" style="width: 95%" :summary-method="getSummaries" show-summary>
        ...
        <el-table-column fixed label="Project Name*" width="200" prop="name">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && rowId == scope.row.id">
        ...
        <el-table-column label="Customer*" width="200">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              <el-select v-model="scope.row.contract_customer_id" filterable track-by="name" value-key="id"
                class="w-100" placeholder="" clearable allow-create default-first-option size="small">
                <el-option v-for="item in customerOptions" :key="item.id" :label="item.name" :value="item.id">
                </el-option>
              </el-select>
            </span>
            <span
              v-if="rowId !== scope.row.id && (scope.row.contract_customer && scope.row.contract_customer.name !== null)">
        ...
        <el-table-column label="Vehicle/ Schedule" width="125">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              ...
              <span
                v-if="(!scope.row.contract_vehicle && scope.row.contract_vehicle_id) || (!scope.row.contract_vehicle && !scope.row.contract_vehicle_id)">
                <el-select v-model="blankVehicle" filterable track-by="name" value-key="id" class="w-100" clearable
                  allow-create placeholder="" default-first-option size="small">
      ...
      (scope.row.contract_vehicle && scope.row.contract_vehicle.name !== null)">
        ...
        <el-table-column label="Contract Number**" width="125">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              <el-select v-model="scope.row.contract_number_id" filterable track-by="name" value-key="id" class="w-100"
                placeholder="" clearable allow-create default-first-option size="small">
                <el-option v-for="item in contractNumber" :key="item.id" :label="item.name" :value="item.id">
                </el-option>
              </el-select>
            </span>
            <span
              v-if="rowId !== scope.row.id && (scope.row.contract_number && scope.row.contract_number.name !== null)">
        ...
        <el-table-column label="Award/ TO Number**" width="125">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              <el-select v-model="scope.row.contract_award_to_id" filterable track-by="id" placeholder="" value-key="id"
                class="w-100" clearable allow-create default-first-option size="small">
      ...
      (scope.row.contract_award_to && scope.row.contract_award_to.name !== null)">
        ...
        <el-table-column label="NAICS*" width="70">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              <el-select v-model="scope.row.contract_naic_id" filterable track-by="name" value-key="id" class="w-100"
                placeholder="" clearable allow-create default-first-option size="small">
        ...
        <el-table-column label="Award Type*" width="70">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              <el-select v-model="scope.row.contract_award_type_id" filterable track-by="name" value-key="id"
                class="w-100" clearable placeholder="" allow-create default-first-option size="small">
                <el-option v-for="item in awardTypes" :key="item.id" :label="item.name" :value="item.id">
                </el-option>
              </el-select>
            </span>
            <span
              v-if="rowId !== scope.row.id && (scope.row.contract_award_type && scope.row.contract_award_type.name !== null)">
        ...
        <el-table-column label="Contract Type*" width="75">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              <el-select v-model="scope.row.contract_type_id" filterable track-by="name" placeholder="" value-key="id"
                class="w-100" clearable allow-create default-first-option size="small">
        ...
        <el-table-column label="Prime vs Sub" width="55" prop="prime_or_sub">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              <el-select v-model="scope.row.prime_or_sub" track-by="name" placeholder="" value-key="id" class="w-100"
                clearable default-first-option size="small">
        ...
        <el-table-column label="Contract Start Date*" width="100" prop="contract_start_date">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
        ...
        <el-table-column label="Contract End Date*" width="100" prop="contract_end_date">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && rowId == scope.row.id">
        ...
        <el-table-column label="PoP's*" width="100">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              <el-select v-model="scope.row.contract_pop_id" filterable track-by="name" value-key="id" class="w-100"
                placeholder="" clearable allow-create default-first-option size="small">
        ...
        <el-table-column label="Current PoP" width="100">
          <template slot-scope="scope">
            <span v-if="_isallowed('write') && (rowId == scope.row.id)">
              <el-select v-model="scope.row.contract_current_pop_id" filterable track-by="name" value-key="id"
                class="w-100" placeholder="" clearable allow-create default-first-option size="small">
            ...
            <span
              v-if="rowId !== scope.row.id && (scope.row.contract_current_pop && scope.row.contract_current_pop.name !== null)">
        ...
        <el-table-column label="Actions" width="145" v-if="_isallowed('write') || _isallowed('delete')" fixed="right"
          align="center">
          <template slot-scope="scope">
            <el-button size="small" type="default" @click="saveContractProject(scope.$index, scope.row)" v-if="scope.$index == rowIndex && (
      ...
      && scope.row.contract_pop_id && popStartDate && popEndDate
            ...
            <el-button size="small" type="default" v-tooltip="`Exempt expiration date`" v-if="scope.$index == rowIndex"
              @click.prevent="setIgnoreStatus(scope.$index, scope.row)" class="bg-light px-2">
              <i class="fa-solid fa-calendar-xmark text-success"></i>
            </el-button>
            <el-button size="small" type="default" v-tooltip="`Edit`" class="bg-light px-2"
              v-if="(scope.$index !== rowIndex) && _isallowed('write')" @click="editMode(scope.$index, scope.row)"><i
                class="fal fa-edit text-primary"></i>
            </el-button>
            <el-button type="default" size="small" v-tooltip="`Delete`" class="bg-light px-2"

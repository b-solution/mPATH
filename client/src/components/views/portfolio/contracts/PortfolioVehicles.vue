    ...
    <el-tabs type="border-card" @tab-click="handleClick">
      <el-tab-pane class="px-3" style="postion:relative" label="PRIME">
        <div style="height:72vh; overflow-y:auto">
          <el-table :data="tableData" :summary-method="getSummaries" show-summary border style="width: 95%">
            <el-table-column fixed label="Prime*" width="215">
              <template slot-scope="scope">
                <el-input tabindex="1" size="small" v-if="(_isallowed('write')) && scope.$index == createRow"
                  placeholder="MicroHealth,LLC" style="text-align:center" v-model="scope.row.prime_name"
                  controls-position="right"></el-input>
                <span v-if="(_isallowed('write')) && rowId == scope.row.id && scope.$index !== createRow">
                  <el-input size="small" placeholder="" style="text-align:center" v-model="scope.row.prime_name"
                    controls-position="right"></el-input>
                </span>
            ...
            <el-table-column fixed label="Vehicle Nick Name*" width="175">
              <template slot-scope="scope">
                <el-input tabindex="1" size="small" v-if="(_isallowed('write')) && scope.$index == createRow"
                  placeholder="" style="text-align:center" v-model="scope.row.name"
                  controls-position="right"></el-input>
                <span v-if="(_isallowed('write')) && rowId == scope.row.id && scope.$index !== createRow">
                  <el-input size="small" placeholder="" style="text-align:center" v-model="scope.row.name"
                    controls-position="right"></el-input>
                </span>
                <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
                  {{ scope.row.name }}
            ...
            <el-table-column label="Vehicle Full Name*" width="300">
              <template slot-scope="scope">
                <el-input tabindex="2" size="small" v-if="(_isallowed('write')) && scope.$index == createRow"
                  placeholder="" style="text-align:center" v-model="scope.row.full_name"
                  controls-position="right"></el-input>
                <span v-if="(_isallowed('write')) && rowId == scope.row.id && scope.$index !== createRow">
                  <el-input size="small" placeholder="" style="text-align:center" v-model="scope.row.full_name"
                    controls-position="right"></el-input>
                </span>
                <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
                  {{ scope.row.full_name }}
            ...
            <el-table-column label="SINS or Subcategories*" width="325" class="">
              <template slot-scope="scope">
                <span v-if="sinsOptions && _isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                  <el-select tabindex="3" v-model="scope.row.contract_sub_category_id" filterable track-by="name"
                    value-key="id" class="w-100" clearable allow-create default-first-option placeholder=""
                    size="small">
                    <el-option v-for="item in sinsOptions" :key="item.id" :label="item.name" :value="item.id">
                    </el-option>
                  </el-select>
                </span>
      ...
      (scope.row.contract_sub_category && scope.row.contract_sub_category.name !== null)">
            ...
            <el-table-column label="Contracting Agency*" width="325">

              <template slot-scope="scope">
                <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                  <el-select tabindex="4" v-model="scope.row.contract_agency_id" filterable track-by="name"
                    value-key="id" class="w-100" clearable allow-create default-first-option placeholder=""
                    size="small">
                    <el-option v-for="item in contractAgencyOptions" :key="item.id" :label="item.name" :value="item.id">
                    </el-option>
                  </el-select>
                </span>
                <span v-if="rowId !== scope.row.id && scope.$index !== createRow &&
      (scope.row.contract_agency && scope.row.contract_agency.name !== null)">
            ...
            <el-table-column label="Vehicle Type*" width="125">
              <template slot-scope="scope">
                <span v-if="_isallowed('write') && (rowId == scope.row.id || scope.$index == createRow)">
                  <el-select tabindex="5" v-model="scope.row.contract_vehicle_type_id" filterable track-by="name"
                    value-key="id" class="w-100" clearable allow-create default-first-option placeholder=""
                    size="small">
                    <el-option v-for="item in vehicleTypes" :key="item.id" :label="item.name" :value="item.id">
                    </el-option>
                  </el-select>
                </span>
                <span v-if="rowId !== scope.row.id && scope.$index !== createRow &&
      (scope.row.contract_vehicle_type && scope.row.contract_vehicle_type.name !== null)">
            ...
            <el-table-column label="CAF Fee" width="175">
              <template slot-scope="scope">
                <el-input tabindex="6" size="small"
                  v-if="(_isallowed('write')) && (rowId == scope.row.id || scope.$index == createRow)" type="number"
                  style="text-align:center" placeholder="" v-model="scope.row.caf_fees"
                  controls-position="right"></el-input>
      ...
      ">
            ...
            <el-table-column label="Contract Number" width="125">
              <template slot-scope="scope">
                <el-input tabindex="7" size="small" v-if="(_isallowed('write')) && scope.$index == createRow"
                  placeholder="" style="text-align:center" v-model="newContractNum"
                  controls-position="right"></el-input>
                <span v-if="(_isallowed('write'))">
                  <el-input size="small"
                    v-if="rowId == scope.row.id && scope.$index !== createRow && scope.row.contract_number"
                    placeholder="" style="text-align:center" v-model="scope.row.contract_number.name"
                    controls-position="right"></el-input>
                  <el-input size="small"
                    v-if="rowId == scope.row.id && scope.$index !== createRow && !scope.row.contract_number"
                    placeholder="" style="text-align:center" v-model="updateContractNum"
                    controls-position="right"></el-input>
                </span>
            ...
            <el-table-column label="Ceiling" width="175" prop="ceiling">
              <template slot-scope="scope">
                <el-input size="small" tabindex="8"
                  v-if="(_isallowed('write')) && (rowId == scope.row.id || scope.$index == createRow)" type="number"
                  style="text-align:center" placeholder="" v-model="scope.row.ceiling"
                  controls-position="right"></el-input>
      ...
      ">
            ...
            <el-table-column label="Base Period Start*" width="100">
              <template slot-scope="scope">
                <v2-date-picker tabindex="9" name="Date" v-if="(_isallowed('write')) && scope.$index == createRow"
                  v-model="newBpStart" value-type="YYYY-MM-DD" format="MM/DD/YYYY" class="w-100" />
                <span v-if="(_isallowed('write')) && rowId == scope.row.id && scope.$index !== createRow">
                  <v2-date-picker name="Date" v-model="bpStart" value-type="YYYY-MM-DD" format="MM/DD/YYYY"
                    class="w-100" />
                </span>
                <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
            ...
            <el-table-column label="Base Period End*" width="100">
              <template slot-scope="scope">
                <v2-date-picker name="Date" tabindex="10" v-if="(_isallowed('write')) && scope.$index == createRow"
                  v-model="newBpEnd" :disabled-date="disabledNewBpEndDate" value-type="YYYY-MM-DD" format="M/DD/YYYY"
                  class="w-100" />
                <span v-if="(_isallowed('write')) && rowId == scope.row.id && scope.$index !== createRow">
                  <v2-date-picker name="Date" v-model="bpEnd" :disabled-date="disabledBpEndDate" value-type="YYYY-MM-DD"
                    format="M/DD/YYYY" class="w-100" />
                </span>
                <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
            ...
            <el-table-column label="Option Period Start" width="100">
              <template slot-scope="scope">
                <v2-date-picker name="Date" tabindex="11" v-if="_isallowed('write') && scope.$index == createRow"
                  v-model="newOpStart" value-type="YYYY-MM-DD" format="M/DD/YYYY" class="w-100" />
                <span v-if="_isallowed('write') && rowId == scope.row.id && scope.$index !== createRow">
                  <v2-date-picker name="Date" v-model="opStart" value-type="YYYY-MM-DD" format="M/DD/YYYY"
                    class="w-100" />
                </span>
                <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
            ...
            <el-table-column label="Option Period End" width="100">
              <template slot-scope="scope">
                <v2-date-picker name="Date" tabindex="12" v-if="scope.$index == createRow && _isallowed('write')"
                  v-model="newOpEnd" :disabled-date="disabledNewOpEndDate" value-type="YYYY-MM-DD" format="M/DD/YYYY"
                  class="w-100" />
                <span v-if="_isallowed('write') && rowId == scope.row.id && scope.$index !== createRow">
                  <v2-date-picker name="Date" v-model="opEnd" :disabled-date="disabledOpEndDate" value-type="YYYY-MM-DD"
                    format="M/DD/YYYY" class="w-100" />
                </span>
                <span v-if="rowId !== scope.row.id && scope.$index !== createRow">
            ...
            <el-table-column label="Actions" v-if="_isallowed('write') || _isallowed('delete')" width="155"
              fixed="right" align="right">
              <template slot-scope="scope">
                <el-button size="mini" type="default" @click="saveContractVehicle(scope.$index, scope.row)" v-if="(_isallowed('write')) && scope.$index == rowIndex && (scope.row.name &&
      scope.row.full_name && scope.row.contract_sub_category_id &&
      scope.row.contract_agency_id && scope.row.contract_vehicle_type_id &&
      bpStart && bpEnd)" v-tooltip="`Save`" class="bg-primary text-light  px-2">
      ...
      scope.row.associated_projects && scope.row.associated_projects.length > 0" placement="left" width="auto"
                  trigger="hover">
                  <el-button v-for="item, i in scope.row.associated_projects" :key="i"
      ...
      item.id == t.program_id)[0].label }}</span>
                  ...
                  <el-button slot="reference" size="mini" type="default" v-tooltip="`Open Vehicle Tasks`"
                    class="bg-light text-light px-2 mr-2">
                    <i class="far fa-suitcase text-secondary"></i>
                  </el-button>
                </el-popover>
                <el-button type="default" size="mini" v-tooltip="`Cancel Edit`" v-if="scope.$index == rowIndex"
                ...
                <el-button size="mini" type="default" v-tooltip="`Edit`" class="bg-light px-2"
                  ...
                  @click="editMode(scope.$index, scope.row)"><i class="far fa-edit text-primary"></i>
                </el-button>
                ...
                <el-button size="mini" type="default" @click="saveContractVehicle(scope.$index, scope.row)" v-if="(_isallowed('write')) && scope.$index == createRow && (checkEmpty(scope.row.name) && checkEmpty(scope.row.full_name) && checkEmpty(scope.row.prime_name) && checkEmpty(scope.row.contract_sub_category_id) && checkEmpty(scope.row.contract_agency_id) && checkEmpty(scope.row.contract_vehicle_type_id) &&
      newBpStart && newBpEnd)" v-tooltip="`Save`" class="bg-primary text-light  px-2">
                  <i class="far fa-save"></i>
                </el-button>
                <el-button size="mini" type="default" v-tooltip="`Cancel`" v-if="(_isallowed('write')) && scope.$index == createRow && (scope.row.prime_name || scope.row.name || scope.row.full_name || scope.row.contract_sub_category_id ||
      scope.row.contract_agency_id || scope.row.contract_vehicle_type_id ||
      ...
      <el-tab-pane class="px-3" style="postion:relative" label="SUBCONTRACT">
        <div style="height:72vh; overflow-y:auto">
          <el-table :data="subTableData" border style="width: 95%">
            <el-table-column fixed label="Subcontract Prime*" width="215" prop="subprime_name">
              <template slot-scope="scope">
                <el-input tabindex="1" size="small" v-if="(_isallowed('write')) && scope.$index == subCreateRow"
                  placeholder="" style="text-align:center" v-model="scope.row.subprime_name"
                  controls-position="right"></el-input>
                <span v-if="(_isallowed('write')) && rowId == scope.row.id && scope.$index !== subCreateRow">
                  <el-input size="small" placeholder="" style="text-align:center" v-model="scope.row.subprime_name"
                    controls-position="right"></el-input>
                </span>
            ...
            <el-table-column fixed label="Vehicle Nick Name*" width="175" prop="name">
              <template slot-scope="scope">
                <el-input tabindex="1" size="small" v-if="(_isallowed('write')) && scope.$index == subCreateRow"
                  placeholder="" style="text-align:center" v-model="scope.row.name"
                  controls-position="right"></el-input>
            ...
            <el-table-column label="Vehicle Full Name*" width="300">
              <template slot-scope="scope">
                <el-input tabindex="2" size="small" v-if="(_isallowed('write')) && scope.$index == subCreateRow"
                  placeholder="" style="text-align:center" v-model="scope.row.full_name"
                  controls-position="right"></el-input>
                <span v-if="(_isallowed('write')) && rowId == scope.row.id && scope.$index !== subCreateRow">
                  <el-input size="small" placeholder="" style="text-align:center" v-model="scope.row.full_name"
            ...
            <el-table-column label="Contract Name*" width="325">

              <template slot-scope="scope">
                <el-input tabindex="2" size="small" v-if="(_isallowed('write')) && scope.$index == subCreateRow"
                  placeholder="" style="text-align:center" v-model="scope.row.contract_name"
                  controls-position="right"></el-input>
                <span v-if="(_isallowed('write')) && rowId == scope.row.id && scope.$index !== subCreateRow">
                  <el-input size="small" placeholder="" style="text-align:center" v-model="scope.row.contract_name"
                    controls-position="right"></el-input>
                </span>
                <span v-if="rowId !== scope.row.id && scope.$index !== subCreateRow">
      ...
      (scope.row.contract_vehicle_type && scope.row.contract_vehicle_type.name !== null)">
            ...
            <el-table-column label="Vehicle/Contract Number" width="175">
              <template slot-scope="scope">
                <el-input tabindex="7" size="small" v-if="(_isallowed('write')) && scope.$index == subCreateRow"
                  placeholder="" style="text-align:center" v-model="newContractNum"
                  controls-position="right"></el-input>
                <span v-if="(_isallowed('write'))">
                  <el-input size="small"
            ...
            <el-table-column label="Actions" v-if="_isallowed('write') || _isallowed('delete')" width="155"
              fixed="right" align="right">
              <template slot-scope="scope">
                <el-button size="mini" type="default" @click="saveContractVehicle(scope.$index, scope.row)" v-if="(_isallowed('write')) && scope.$index == rowIndex && (scope.row.subprime_name && scope.row.name &&
      scope.row.full_name && scope.row.contract_agency_id && scope.row.contract_vehicle_type_id)" v-tooltip="`Save`"
      ...
      item.id == t.program_id)[0].label }}</span>
                ...
                <el-button size="mini" type="default" v-tooltip="`Edit`" class="bg-light px-2"
                  ...
                  @click="editMode(scope.$index, scope.row)">
                ...
                <el-button type="default" size="mini" @click="saveContractVehicle(scope.$index, scope.row)"
                  v-if="(_isallowed('write')) && scope.$index == subCreateRow && (checkEmpty(scope.row.subprime_name) && checkEmpty(scope.row.name) &&
      checkEmpty(scope.row.full_name) && checkEmpty(scope.row.contract_agency_id) && checkEmpty(scope.row.contract_vehicle_type_id))" v-tooltip="`Save`" :load="log(scope.row)"
                  class="bg-primary text-light  px-2">
                  <i class="far fa-save"></i>
                </el-button>
                <el-button size="mini" type="default" v-tooltip="`Cancel`"
